...
class Sms_SettingsController extends Controller
{
    ...
    public function get_sms_config(Request $request)
    {
		...
		if ($user_auth->can('sms_settings')){
            ...
            $default_sms_gateway = $settings->default_sms_gateway;

            return view('settings.sms_settings', compact('twilio','nexmo','infobip','default_sms_gateway'));
    ...
    public function update_Default_SMS(Request $request)
    {
		...
		if ($user_auth->can('sms_settings')){
            ...
            $settings = Setting::where('deleted_at', '=', null)->first();
            $settings->default_sms_gateway =  $request['default_sms_gateway'];
            $settings->save();
