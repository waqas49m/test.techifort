...
class ProductsController extends Controller
{
    ...
    {
        $amount = floatval($request->current_value);
        $fromCurrency = 'GBP';
        ...
        if (!$user_auth->can('products_view')) {
            ...
            foreach ($products as $product) {
                ...
                if ($product->type == 'is_single') {
                    ...
                    $item['name'] = $product->name;
                    $item['ean_code'] = $product->ean_code;
                    $item['buying_price']  = $this->render_price_with_symbol_placement(number_format($product->buying_price, 2, '.', ','), $symbol_placement);
                    $item['rrp'] = $this->render_price_with_symbol_placement(number_format($product->rrp, 2, '.', ','), $symbol_placement);
    ...
    public function create_kit()
    {
        ...
        if ($user_auth->can('products_add')) {
            ...
            return view('products.create_product_kit', compact('categories', 'brands', 'units'));
        }
        return abort('403', __('You are not authorized'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($user_auth->can('products_add')) {
            ...
            \DB::transaction(function () use ($request) {
                ...
                $Product->community_code         = $request['community_code'];
                ...
                if ($request->hasFile('product_artwork')) {

                    $artFile = $request->file('product_artwork');
                    $artfilename = time() . '.' . $artFile->extension();
                    $artFile->move(public_path('/images/products/artwork'), $artfilename);
                } else {
                    $artfilename = '';
                }

                $Product->product_artwork = $artfilename;
                $Product->save();
    ...
    public function import_products(Request $request)
    {
            ...
            if ($file->isValid()) {
                try {
                    ...
                    foreach ($failures as $failure) {
                        $productName = $failure->values()['name'];
                        // Store error message in session
                        $request->session()->push('import_errors', 'Product ' . $productName . ' already exists.');
                    }

                    // Redirect back with error message
                    return redirect()->back()->with('error', 'There were some errors during import.');
                }
            } else {
        ...
        //     $product_array = Excel::toArray(new ProductImport, $request->file('$request'));
    ...
    {
        return view('products.print_labels_bulk');
    }

   public function generate_labels_bulk(Request $request)
    {
        $file = $request->file('label_file');
        $data = Excel::toArray(new LabelImport, $file);
        $filteredArray1 = [];
        $filteredArray2 = [];

        for ($i = 1; $i < count($data[0]); $i++) {
            if (!preg_match('/^(KEC|LVH|KDE|KLB|KWL)/', $data[0][$i][1])) {
                $filteredArray1[] = $data[0][$i];
            } else {
                for ($j = 0; $j < $data[0][$i][4]; $j++) {
                    $filteredArray2[] = $data[0][$i];
                }
            }
        }
        return view('products.labels', compact('filteredArray1', 'filteredArray2'));
    }
}
