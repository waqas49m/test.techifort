...
<div class="row" id="section_edit_purchase_return">
  <div class="col-lg-12 mb-3">
    <validation-observer ref="edit_Return">
      <form @submit.prevent="Submit_purchase_return">
                  ...
                  <table class="table table-striped table-sm">
                    <tbody>
                      <tr>
                        ...
                        <td>
                          <span>{{$currency}} @{{purchase_return.TaxNet.toFixed(2)}}
                            (@{{formatNumber(purchase_return.tax_rate,2)}}
                            %)</span>
                        </td>
                      </tr>
                      <tr>
                        <td class="bold">{{ __('translate.Discount') }}</td>
                        <td v-if="purchase_return.discount_type == 'fixed'"><span>{{$currency}}
                            @{{purchase_return.discount.toFixed(2)}}</span></td>
                        <td v-else> <span>{{$currency}} @{{purchase_return.discount_percent_total.toFixed(2)}}
                            (@{{formatNumber(purchase_return.discount,2)}} %)</span></td>
                      </tr>
                      <tr>
        ...
        <div class="row mt-3">
          <div class="col-lg-6">
            <button @click="Submit_purchase_return" class="btn btn-primary" :disabled="SubmitProcessing">
...
<script>
  ...
  Vue.component('validation-observer', VeeValidate.ValidationObserver);
    ...
    var app = new Vue({
        el: '#section_edit_purchase_return',
        data: {
          ...
          total: 0,
          GrandTotal: @json($purchase_return['GrandTotal']),
        },
      ...
      for (var i = 0; i < this.details.length; i++) {
        if (this.details[i].detail_id == id) {
          ...
          if (detail.quantity > detail.purchase_quantity) {
            ...
            toastr.error('{{ __('translate.qty_return_is_greater_than_Quantity_Remaining') }}');
            this.details[i].quantity = detail.quantity_copy;
          } else {
   ...
   verifiedForm() {
    ...
    Update_purchase_return() {
      if (this.verifiedForm()) {
        ...
        NProgress.set(0.1);
        axios
        .put('/purchase-return/returns_purchase/'+ this.purchase_return.id, {
